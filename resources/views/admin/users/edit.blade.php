...
@section('content')
    ...
    <form action="" method="post">
        ...
        <div class="form-group">
            ...
            <input type="text" name="name" value="{{old('name')??$user->name}}" class="form-control @error('name') is-invalid @enderror" placeholder="Nhập họ tên...">
            @error('name')
        ...
        <div class="form-group">
            ...
            <input type="text" name="email" value="{{old('email')??$user->email}}" class="form-control @error('email') is-invalid @enderror" placeholder="Nhập email...">
            @error('email')
                <small style="color: red; font-size: 15px;font-style: italic">{{ $message}}</small>     
            @enderror
        </div>
        <div class="form-group">
            <label for="exampleInputEmail1">Mật khẩu (Không nhập nếu không đổi)</label>
        ...
        <div class="form-group">
            ...
            <select name="group_id" class="custom-select @error('group_id') is-invalid @enderror">
                ...
                @foreach ($groups as  $item)
                    <option  value="{{$item->id}}" {{old('group_id') || $user->group_id == $item->id?'selected':false}}>{{$item->name}}</option>
                @endforeach
              </select>
            @error('group_id')
                <small style="color: red; font-size: 15px;font-style: italic">{{ $message}}</small>     
